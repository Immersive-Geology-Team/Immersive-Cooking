...
public class ModRecipeProvider extends net.minecraft.data.recipes.RecipeProvider {
    ...
    public static void registerFarmersRecipes(Consumer<FinishedRecipe> consumer)
    {
        ...
        for(CuttingRecipeRecord record : cuttingRecipes)
        {
            ...
            if(record.extra != null) builder.addResult(record.extra, record.count_extra);
        ...
        grillRecipe("potato_slice_to_fried_potato_slice", ItemRegistry.ITEM_MAP.get(FoodStuff.POTATO_SLICE.getRegistryName()).get().asItem(), ItemRegistry.ITEM_MAP.get(FoodStuff.FRIED_POTATO_SLICE.getRegistryName()).get().asItem(), .35f, 600, consumer);
        grillRecipe("potato_cube_to_fried_potato_cube", ItemRegistry.ITEM_MAP.get(FoodStuff.POTATO_CUBES.getRegistryName()).get().asItem(), ItemRegistry.ITEM_MAP.get(FoodStuff.FRIED_POTATO_CUBES.getRegistryName()).get().asItem(), .35f, 600, consumer);
        foodSmeltingRecipes("cooked_spider_cutlet", ItemRegistry.ITEM_MAP.get(FoodStuff.RAW_SPIDER_CUTLET.getRegistryName()).get().asItem(), ItemRegistry.ITEM_MAP.get(FoodStuff.COOKED_SPIDER_CUTLET.getRegistryName()).get().asItem(), .35f, consumer);
        smokeRecipe("smoked_shank", ItemRegistry.ITEM_MAP.get(FoodStuff.CLEAN_SPIDER_SHANK.getRegistryName()).get().asItem(), ItemRegistry.ITEM_MAP.get(FoodStuff.SMOKED_SPIDER_SHANK.getRegistryName()).get().asItem(), .35f, consumer);
    ...
    private static void initializeFoodData() {
        ...
        // SPIDER_CUTLET
        new CuttingRecipeRecord(Ingredient.of(ForgeTags.TOOLS_KNIVES),
        ...
        new CookingPotRecord(ItemRegistry.ITEM_MAP.get(FoodStuff.PYTTIPANNA.getRegistryName()).get(), 2, 100, 0.35f, new SizedIngredient[]{
                ...
                new SizedIngredient(Ingredient.of(ForgeTags.RAW_BEEF), 1),
                new SizedIngredient(Ingredient.of(ForgeTags.RAW_BACON), 1),
                new SizedIngredient(Ingredient.of(ForgeTags.RAW_PORK), 1),
        }, Items.BOWL);
        ...
        new CookingPotRecord(ItemRegistry.ITEM_MAP.get(FoodStuff.CURRY_BASE.getRegistryName()).get(), 4, 200, 0.35f, new SizedIngredient[]{
                ...
                new SizedIngredient(Ingredient.of(ForgeTags.VEGETABLES_TOMATO), 2),
    ...
    private static void foodSmeltingRecipes(String name, ItemLike ingredient, ItemLike result, float experience, Consumer<FinishedRecipe> consumer) {
        ...
        grillRecipe(name, ingredient, result, experience, 600,consumer);
        smokeRecipe(name, ingredient, result, experience,consumer);
    }

    private static void smokeRecipe(String name, ItemLike ingredient, ItemLike result, float experience, Consumer<FinishedRecipe> consumer) {
    ...
    private static void grillRecipe(String name, ItemLike ingredient, ItemLike result, float experience, int timer, Consumer<FinishedRecipe> consumer) {
        String namePrefix = new ResourceLocation(ImmersiveCooking.MODID, name).toString();
