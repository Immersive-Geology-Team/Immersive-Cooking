package crimson_twilight.immersive_cooking.tile;
...
public class PantryCounterTileEntity extends LockableLootTileEntity
{
    ...
    protected ITextComponent getDefaultName() {
        return new TranslationTextComponent("container.pantry");
    }
    ...
    public void startOpen(PlayerEntity player) {
        if (!player.isSpectator()) {
            ...
            boolean flag = blockstate.getValue(PantryCounter.OPEN);
            if (!flag) {
    ...
    public void recheckOpen() {
        ...
        if (this.openCount > 0) {
            ...
            BlockState blockstate = this.getBlockState();
            if (!(blockstate.getBlock() instanceof PantryCounter)) {
                this.setRemoved();
                return;
            }
    ...
    private void updateBlockState(BlockState state, boolean open) {
        this.level.setBlock(this.getBlockPos(), state.setValue(PantryCounter.OPEN, Boolean.valueOf(open)), 3);
    }

    private void playSound(BlockState p_213965_1_, SoundEvent p_213965_2_) {
        Vector3i vector3i = p_213965_1_.getValue(PantryCounter.FACING).getNormal();
